...
(defmacro check (function-form &key message)
  (if *compile-checked*
	...
	`(let* ((cookie (,checked-function-symbol ,@(cdr function-form)))
                (ret (request-check ,(cadr function-form) cookie)))
	   (unless (cffi:null-pointer-p ret)
	     (error ,message))))
      function-form))
...
(defmacro defxcb ((cname lispname) &body body)
  (let ((cname-checked (concatenate 'string cname "_checked"))
    ...
    `(progn
       (cffi:defcfun (,cname ,lispname) :uint32
	 (c        :pointer)
	 ,@body)
       (cffi:defcfun (,cname-checked ,lispname-checked) :uint32
	 (c        :pointer)
	 ,@body))))
...
(defmacro foreign-values (&rest values)
  (multiple-value-bind (ts cts vals) (cffi::parse-args-and-types values)
	...
	(loop for type in cts
	   for size = (cffi:foreign-type-size type)
	 ...
	 ,@(loop for type in cts
	      ...
	      collect `(setf (cffi:mem-ref temp ,type ,offset) ,val))
	 temp))))
