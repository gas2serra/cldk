...
(defun make-pixeled-uniform-design (&key (red 0) (green 0) (blue 0) (alpha 255))
  (make-instance 'pixeled-uniform-design :red red :green green :blue blue :alpha alpha :region +everywhere+))
...
(defmethod  pixeled-design-rgba-get-fn ((design pixeled-image-design))
  (with-slots (image dx dy region)
      design
    (image-rgba-get-fn image :dx dx :dy dy)))
...
(defclass pixeled-rgba-pattern (pixeled-design)
  ((pattern :initarg :pattern :initform nil
            :accessor pixeled-pattern-design-pattern)
   (dx :initarg :dx :initform 0 :type fixnum
       :accessor pixeled-pattern-design-dx)
   (dy :initarg :dy :initform 0 :type fixnum
       :accessor pixeled-pattern-design-dy)))


(defun make-pixeled-rgba-pattern (&key (pattern nil))
  (make-instance 'pixeled-rgba-pattern
                 :pattern pattern
                 :region (make-rectangle* 0 0 (1- (pattern-width pattern)) (1- (pattern-height pattern)))))

(defmethod  pixeled-design-rgba-get-fn ((design pixeled-rgba-pattern))
  (with-slots (pattern dx dy region)
      design
    (lambda (x y)
      #+nil (log:warn "~A" (multiple-value-list (mcclim-render-internals::%rgba->vals
                      (climi::%pattern-rgba-value pattern x y))))
      (mcclim-render-internals::%rgba->vals
       (climi::%pattern-rgba-value pattern x y)))))

(defmethod  pixeled-design-rgba-get-unsafe-fn ((design pixeled-rgba-pattern))
  (with-slots (pattern dx dy region)
      design
    (lambda (x y)
      (mcclim-render-internals::%rgba->vals
       (climi::%pattern-rgba-value pattern x y)))))
#|
(defclass pixeled-pattern-design (pixeled-design)
  ((pattern :initarg :pattern :initform nil
            :accessor pixeled-pattern-design-pattern)
   (dx :initarg :dx :initform 0 :type fixnum
       :accessor pixeled-pattern-design-dx)
...
(defun make-pixeled-pattern-design (&key (pattern nil))
  (make-instance 'pixeled-pattern-design
                 :pattern pattern
                 :region (make-rectangle* 0 0 (1- (pattern-width pattern)) (1- (pattern-height pattern)))))
|#
#|
(defmethod  pixeled-design-rgba-get-fn ((design pixeled-pattern-design))
  (with-slots (pattern dx dy region)
      design
    (lambda (x y) (mcclim-render-internals::%rgba->vals (climi::%pattern-rgba-value pattern x y)))))

(defmethod  pixeled-design-rgba-get-unsafe-fn ((design pixeled-pattern-design))
  (with-slots (pattern dx dy region)
      design
    (lambda (x y) (mcclim-render-internals::%rgba->vals (climi::%pattern-rgba-value pattern x y)))))
|#
...
(defmethod %make-pixeled-design ((ink clim-extensions:image-pattern))
  (make-pixeled-rgba-pattern :pattern ink))
