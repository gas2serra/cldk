...
(defun x-keysym-to-modifiers (port event-key keychar keysym-name state)
  ...
  (multiple-value-bind (modifiers caps-lock? mode-switch?)
      (x-event-state-modifiers port state)
    (declare (ignore caps-lock? mode-switch?))
    (if (characterp keychar)
	modifiers
	(modify-modifiers event-key keysym-name modifiers))))
...
;;; switches.
(defun create-modifier-mask (clx-modifier-mask)
...
(defun make-modifier-cache (driver)
    ...
    (let* ((cache (make-array 256))
      ...
      (loop for x-modifier-mask from 0 below 256
         do (setf (aref cache x-modifier-mask)
                  (cons (create-modifier-mask x-modifier-mask)
...
(defmethod x-event-state-modifiers ((driver keysym-driver-mixin) state)
  (with-accessors ((modifier-cache modifier-cache))
      driver
    (unless modifier-cache
      (setf modifier-cache (make-modifier-cache driver)))
    (destructuring-bind (modifiers . other-modifiers)
...
(defun modify-modifiers (event-key keysym-name modifiers)
  (let ((keysym-modifier (loop for (keysyms modifier) in +modifiers+
