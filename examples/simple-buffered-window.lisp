...
(defun fill-window (w r g b)  
  (multiple-value-bind (width height)
    ...
    (let ((img (make-instance 'cldki::rgb-image :width width :height height)))
      ...
      ;; copy
      (cldki::copy-image-to-buffered-window w img 0 0 width height)
...
(defun simple-buffered-window-02 (backend)
  (let ((*s* (cldk:find-display-server :server-path backend)))
    (let ((w (cldk:create-buffered-window *s* "Pippo" :mode :managed)))
      (cldk:show-window w)
      (fill2-window w 255 0 0)
      (sleep 1)
      (fill2-window w 0 255 0)
      (sleep 1)
      (fill2-window w 0 0 255)
      (sleep 1)
      (cldk:destroy-window w))))
